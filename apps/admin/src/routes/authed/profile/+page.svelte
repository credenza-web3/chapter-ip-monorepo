  ...
  <div class="w-full">
    ...
    <div class="flex flex-col gap-6 w-full justify-between">
      <div class="max-w-1/2 gap-2 bg-[#f9fafb] p-6 rounded-lg border-[#dddddd] border">
        ...
        <div class="flex justify-between items-center text-sm font-mono bg-white p-2 rounded break-all border-2 border-[#e5e5e5]">
          <div>
<img src={blockIcon} alt="block" class="w-8 h-8 inline-block" />
       ...
       <div class="flex flex-col items-end gap-2 w-1/2 mt-10">
          <EditPublisher 
            onUpdate={profileSave.updatePublisherData}
          />
          <SubscriptionPriceInput 
            onUpdate={profileSave.updateSubscriptionPrice} 
            hideSaveButton={true} 
          />
        </div>

       
      <AgencyControls contentContract={data.contentContract} userAddress={data.userAddress as string} hideSaveButtons={true} />
