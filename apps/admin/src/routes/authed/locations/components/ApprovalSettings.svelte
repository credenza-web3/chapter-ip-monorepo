            ...
            <button
              ...
              onclick={() => {
                if (question.key === 'allowRetouching') {
                  locationStore.setAllowRetouching(option as 'yes' | 'no')
                } else {
                  locationStore.setApproveFinalUse(option as 'yes' | 'no')
                }
              }}
              class="w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors
                {$locationStore.licensing[question.key] === option ? 'border-primary' : 'border-[#ddd4cc]'}"
            >
              {#if $locationStore.licensing[question.key] === option}
