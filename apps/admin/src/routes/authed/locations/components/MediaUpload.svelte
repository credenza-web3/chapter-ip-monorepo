<script lang="ts">
  ...
  let {
    label,
    required = false,
    fileKey = 'locations' as LocationFileKey,
    multiple = true,
  } = $props<{
    label: string
    required?: boolean
    fileKey?: LocationFileKey
    multiple?: boolean
  }>()

  const accept = 'image/*'
  const helperText = '.avif, .gif, .jpg, .png, .svg, .webp files accepted'
  ...
  function appendFiles(files: File[]) {
    ...
    const allValid = files.every((file) => file.type.startsWith('image/'))
    ...
    locationStore.appendMediaFiles(fileKey, files)
  }
  ...
  function removeFile(e: MouseEvent, index: number) {
    e.stopPropagation()
    locationStore.removeMediaFile(fileKey, index)
  }

  function removeExisting(e: MouseEvent, index: number) {
    e.stopPropagation()
    locationStore.removeExistingFile(fileKey, index)
  }
</script>
...
<div class="space-y-1.25 w-full">
  ...
  >
    ...
    {:else}
      <p class="text-base font-semibold text-dark">Upload or drag your {label.toLowerCase()}</p>
      <button
        type="button"
        onclick={openFilePicker}
