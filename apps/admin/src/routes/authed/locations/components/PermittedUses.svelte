<script lang="ts">
  ...
  let allPermitted = $derived(PERMITTED_USES.every((u) => $locationStore.licensing.permittedUses[u.id]))

  function toggleAllUses() {
    locationStore.setAllPermittedUses(
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6">
    {#each PERMITTED_USES as use (use.id)}
      <div class="flex items-start gap-3">
        <Toggle
          checked={$locationStore.licensing.permittedUses[use.id]}
          onToggle={() => locationStore.setPermittedUse(use.id, !$locationStore.licensing.permittedUses[use.id])}
        />
        <div>
